...
export default function LegalPageShell({ title, lastUpdated, children }) {
    return (
        <div className="min-h-screen flex flex-col bg-[var(--color-bg)] text-[var(--color-text)] transition-colors duration-300">
            <Navbar />
            <main className="flex-1 container max-w-3xl mx-auto px-4 pt-24 md:pt-28 pb-16">
                <Link
                    to="/"
                    className="inline-block text-sm font-semibold text-[var(--color-primary)] hover:underline dark:text-emerald-400 mb-8"
                >
                    ← Back to home
                </Link>
                ...
                <div className="mt-10 space-y-8 text-[15px] md:text-base leading-relaxed text-slate-800 dark:text-slate-200">
                    {children}
                </div>
            </main>
            <SiteFooter />
