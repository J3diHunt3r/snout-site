...
const ParallaxBackground = () => {
    ...
    return (
        <div className="fixed inset-0 pointer-events-none overflow-hidden -z-10">
            {/* Floating Paw 1 */}
            <motion.div style={{ y: y1 }} className="absolute top-[20%] right-[10%] text-[var(--color-primary)] opacity-5 dark:opacity-10 rotate-12">
                <PawPrint size={200} />
            </motion.div>

            {/* Floating Paw 2 */}
            <motion.div style={{ y: y2 }} className="absolute top-[50%] left-[5%] text-[var(--color-secondary)] opacity-5 dark:opacity-10 -rotate-12">
                <PawPrint size={150} />
            </motion.div>

            {/* Geometric Shape 1 */}
            <motion.div style={{ y: y3 }} className="absolute top-[80%] right-[20%] text-gray-300 dark:text-gray-700 opacity-20 dark:opacity-10">
                <Hexagon size={100} />
            </motion.div>
